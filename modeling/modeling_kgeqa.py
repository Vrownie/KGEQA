...
class KGEQA_Message_Passing(nn.Module):
...
class KGEQA(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.gnn = KGEQA_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
...
class LM_KGEQA_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
        ...
        self.train_qids, self.train_labels, *self.train_encoder_data = load_input_tensors_for_kgeqa(train_statement_path, model_type, model_name, max_seq_length)
        self.dev_qids, self.dev_labels, *self.dev_encoder_data = load_input_tensors_for_kgeqa(dev_statement_path, model_type, model_name, max_seq_length)
        ...
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice, args)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, *self.test_encoder_data = load_input_tensors_for_kgeqa(test_statement_path, model_type, model_name, max_seq_length)
